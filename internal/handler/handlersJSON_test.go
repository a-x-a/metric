...
func sendTestRequest(t *testing.T, method, path string, data []byte) *http.Response {
	rt := NewRouter(mockService{}, zap.L())
...
func TestGetJSONMetricWithMockStorage(t *testing.T) {
	type result struct {
		code int
		body adapter.RequestMetric
	}
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			resp := sendTestRequestWithMocStorage(t, http.MethodPost, "/value/", data)
			assert.Equal(tc.expected.code, resp.StatusCode)
